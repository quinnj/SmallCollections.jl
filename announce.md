# SmallCollections.jl: fast small vectors, sets and dictionaries

The standard container types in Julia (`Vector`, `Set`, `Dict`) can hold an
arbitrary number of elements. If the size is known in advance, one can use
dedicated types like `SVector` or `MVector` from StaticArrays.jl
that are much faster and, in the case of `SVector`, don't allocate any memory.
In many problems, however, the size of vectors or sets is not fixed,
but instead known to be "small". In enumerative combinatorics, for example,
combinatorial explosions make it often impossible to reach "large" vectors or sets.
...
for full functionality (as does `MVector`).
This approach leads to significant speed-ups compared to the standard types.
For instance, the performance of the vector types `SmallVector` and `MutableSmallVector`
is close to that to `SVector` and `MVector`. The most extreme performance gain is
with `SmallBitSet`, which can be 100x faster than `BitSet`.

Below we present the new types and also the submodule `Combinatorics`.
Compared to their counterparts in Oscar.jl, GAP and SageMath,
the functions in `Combinatorics` can be 2 or 3 orders of magnitude faster.
...
The types
[`SmallVector{N,T}`](https://matthias314.github.io/SmallCollections.jl/dev/capacityvector/#SmallCollections.SmallVector)
and
[`MutableSmallVector{N,T}`](https://matthias314.github.io/SmallCollections.jl/dev/capacityvector/#SmallCollections.MutableSmallVector)
...
[`default`](https://matthias314.github.io/SmallCollections.jl/dev/nonexported/#SmallCollections.default)
value. The underlying types
[`FixedVector{N,T}`](https://matthias314.github.io/SmallCollections.jl/dev/fixedvector/#SmallCollections.FixedVector)
and
[`MutableFixedVector{N,T}`](https://matthias314.github.io/SmallCollections.jl/dev/fixedvector/#SmallCollections.MutableFixedVector)
...
[`map`](https://matthias314.github.io/SmallCollections.jl/dev/capacityvector/#Base.map)
or
[`any`](https://matthias314.github.io/SmallCollections.jl/dev/capacityvector/#Base.any-Tuple{Function,%20AbstractSmallVector})
deal with default values for a `(Mutable)SmallVector` is governed by a trait called
[`MapStyle`](https://matthias314.github.io/SmallCollections.jl/dev/nonexported/#SmallCollections.MapStyle).

### `PackedVector`

A
[`PackedVector{U,M,T}`](https://matthias314.github.io/SmallCollections.jl/dev/capacityvector/#SmallCollections.PackedVector)
...
can store `25` (`128÷5`) signed integers with `5` bits.
Besides saving memory, `PackedVector` may be faster for operations like `pushfirst` or `pop`.

### `(Mutable)SmallSet` and `SmallBitSet`

The types
[`SmallSet{N,T}`](https://matthias314.github.io/SmallCollections.jl/dev/smallset/#SmallCollections.SmallSet)
and
[`MutableSmallSet{N,T}`](https://matthias314.github.io/SmallCollections.jl/dev/smallset/#SmallCollections.MutableSmallSet)
 can hold up to `N` elements of type `T`. The type
[`SmallBitSet{U}`](https://matthias314.github.io/SmallCollections.jl/dev/smallbitset/#SmallCollections.SmallBitSet)
...
The types
[`SmallDict{N,K,V}`](https://matthias314.github.io/SmallCollections.jl/dev/smalldict/#SmallCollections.SmallDict)
and
[`MutableSmallDict{N,K,V}`](https://matthias314.github.io/SmallCollections.jl/dev/smalldict/#SmallCollections.MutableSmallDict)
...
[`invget`](https://matthias314.github.io/SmallCollections.jl/dev/smalldict/#SmallCollections.invget)
...
The submodule
[`Combinatorics`](https://matthias314.github.io/SmallCollections.jl/dev/combinatorics/)
...
[`permutations`](https://matthias314.github.io/SmallCollections.jl/dev/combinatorics/#SmallCollections.Combinatorics.permutations)
...
The iterator returned by
[`subsets`](https://matthias314.github.io/SmallCollections.jl/dev/combinatorics/#SmallCollections.Combinatorics.subsets-Tuple{Integer,%20Integer})
